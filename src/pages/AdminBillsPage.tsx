    ...
    switch (p) {
      ...
      case "yesterday": {
        ...
        return {
          ...
          endDate: new Date(y.getTime() + 86400000 - 1).toISOString(),
        };
      }
      case "thisWeek": {
        const d = today.getDay();
        return {
          startDate: new Date(today.getTime() - d * 86400000).toISOString(),
  ...
  const handleDelete = async (billId: string) => {
    ...
    try {
      await api.delete(`/bills/${billId}`, {
        data: { reason: deleteReason || "Admin deletion" },
      });
              ...
              {expandedBill === bill._id && (
                <div className='border-t border-gray-100 px-4 py-3 bg-gray-50/50 space-y-3'>
                  ...
                  <div className='text-[10px] text-gray-400'>
                    Vehicle:{" "}
                    {typeof bill.vehicleId === "object"
                      ? bill.vehicleId.registrationNumber
                      : ""}{" "}
              ...
              <div className='bg-gray-50 rounded-xl p-3'>
                <p className='text-xs text-gray-500'>
                  ...
                  <span className='font-medium text-gray-800'>
                    {typeof editingBill.shopId === "object"
                      ? editingBill.shopId.name
                      : ""}
                  </span>
                </p>
              </div>
                  ...
                  <div
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (p) {
                          n[idx].sellingPrice = p.defaultSellingPrice;
                          n[idx].costPriceSnapshot = p.costPrice;
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...editItems];
                        n[idx].sellingPrice = Number(e.target.value);
                        setEditItems(n);
                      }}
                    />
                  ...
                  onClick={() =>
                    setEditItems([
                      ...editItems,
                      {
                        ...
                        quantity: 1,
                        sellingPrice: products[0]?.defaultSellingPrice || 0,
                        costPriceSnapshot: products[0]?.costPrice || 0,
                      },
                    ])
                  }
              ...
              <div className='border-t border-gray-100 pt-3'>
                <div className='flex justify-between text-sm font-bold mb-3'>
                  ...
                  <span>
                    ₹
                    {editItems
                      .reduce((a, i) => a + i.quantity * i.sellingPrice, 0)
                      .toLocaleString()}
                  </span>
                </div>
          ...
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-sm'>
            ...
            <div className='p-5 space-y-4'>
              <p className='text-sm text-gray-600'>
                This bill will be soft-deleted. Outstanding balance will be
              ...
              {auditLog.map((log: any, i: number) => (
                <div key={i} className='bg-gray-50 rounded-xl p-3 space-y-1.5'>
                  <div className='flex justify-between items-center'>
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded ${log.action === "DELETE" ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"}`}>
                      {log.action}
                    </span>
                  ...
                  {log.action === "EDIT" && log.originalData && (
                    <div className='text-[10px] text-gray-400 space-y-0.5'>
                      <p>
                        Original: ₹{log.originalData.totalAmount} (Paid: ₹
                        {log.originalData.paidAmount})
                      </p>
                      {log.modifiedData && (
                        <p>
                          Modified: ₹{log.modifiedData.totalAmount} (Paid: ₹
                          {log.modifiedData.paidAmount})
                        </p>
                      )}
                    </div>
                  )}
