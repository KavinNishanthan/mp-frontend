...
const ShopsPage = () => {
  ...
  const fetchData = async () => {
    try {
      const [shopsRes, productsRes] = await Promise.all([shopsApi.getAll(), productsApi.getAll()]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const payload = { ...form, customPrices: form.customPrices.filter((cp) => cp.price > 0) };
      if (editingId) {
        await shopsApi.update(editingId, payload);
      } else {
      ...
      const message =
        typeof err === 'object' && err !== null && 'response' in err
          ? (err as { response?: { data?: { message?: string } } }).response?.data?.message ?? 'Operation failed'
          : 'Operation failed';
      setError(message);
    }
  };
  ...
  const getCustomPrice = (productId: string): number =>
    form.customPrices.find((cp) => cp.productId === productId)?.price ?? 0;
          ...
          <table className='min-w-full text-left text-sm'>
            <thead>
              <tr className='bg-white border-b'>
                ...
                <th className='px-6 py-3 font-semibold text-gray-600'>Address</th>
                <th className='px-6 py-3 font-semibold text-gray-600'>Contact</th>
                <th className='px-6 py-3 font-semibold text-gray-600'>Outstanding</th>
                <th className='px-6 py-3 font-semibold text-gray-600'>Custom Prices</th>
              ...
              {shops.map((shop, i) => (
                <React.Fragment key={shop._id}>
                  <tr className={`${i % 2 ? 'bg-gray-100' : 'bg-white'}`}>
                    ...
                    <td className='px-6 py-4 text-gray-600 max-w-xs truncate'>{shop.address}</td>
                    <td className='px-6 py-4 text-gray-600'>{shop.contactNumber ?? '—'}</td>
                    ...
                    <td className='px-6 py-4'>
                      <button
                        onClick={() => setShowPricing(showPricing === shop._id ? null : shop._id)}
                        className='text-blue-600 hover:underline text-xs'>
                        {shop.customPrices.length > 0 ? `${shop.customPrices.length} override(s)` : 'None'}
                      </button>
                    </td>
                    ...
                    <tr className='bg-blue-50/50'>
                      <td colSpan={6} className='px-6 py-3'>
                        <div className='text-xs space-y-1'>
                          <p className='font-semibold text-gray-700 mb-2'>Custom Price Overrides:</p>
                          {products.map((p) => {
                            const cp = getShopCustomPrice(shop, p._id);
                            if (!cp) return null;
                            return (
                              <div key={p._id} className='flex gap-4'>
                                <span className='text-gray-600'>{p.name} ({p.unit})</span>
                                <span className='font-medium'>₹{cp}</span>
                                <span className='text-gray-400'>(Default: ₹{p.defaultSellingPrice})</span>
                              </div>
                            );
                          })}
                          {shop.customPrices.length === 0 && (
                            <span className='text-gray-400'>No overrides set</span>
                          )}
                        </div>
                      </td>
                    </tr>
        ...
        <div className='fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4'>
          <div className='bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto'>
            <div className='flex justify-between items-center p-5 border-b sticky top-0 bg-white rounded-t-xl'>
              <h2 className='text-lg font-bold text-gray-800'>{editingId ? 'Edit Shop' : 'Add Shop'}</h2>
                  ...
                  <div className='space-y-2'>
                    {products.map((p) => (
                      <div key={p._id} className='flex items-center gap-3 text-sm'>
                        <span className='flex-1 text-gray-600'>{p.name} ({p.unit})</span>
                        <span className='text-gray-400 text-xs'>Default: ₹{p.defaultSellingPrice}</span>
                        <input
                          type='number'
                          step='0.01'
                          min='0'
                          value={getCustomPrice(p._id) || ''}
                          onChange={(e) => updateCustomPrice(p._id, Number(e.target.value))}
