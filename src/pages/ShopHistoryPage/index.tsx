...
interface Transaction {
  ...
  items?: { productId?: { name: string }; quantity: number; total: number }[];
  isDeleted?: boolean;
...
const ShopHistoryPage = () => {
  ...
  const getDateRange = (p: DatePreset): { startDate?: string; endDate?: string } => {
    ...
    switch (p) {
      ...
      case 'thisWeek': {
        ...
        return { startDate: new Date(today.getTime() - day * 86400000).toISOString(), endDate: now.toISOString() };
      }
      case 'thisMonth':
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const { data: d } = await shopsApi.getHistory(id!, params);
      setData(d);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className='space-y-6 animate-fade-in'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-3'>
        {[
          { label: 'Current Outstanding', value: `₹${(data?.currentOutstanding ?? 0).toLocaleString()}`, color: 'text-red-600' },
          { label: 'Total Bills', value: data?.totalBills ?? 0, color: 'text-indigo-600' },
          { label: 'Total Payments', value: data?.totalPayments ?? 0, color: 'text-green-600' },
      ...
      <div className='space-y-3'>
        ...
        {transactions.map((t) => (
          <div
            key={`${t.type}-${t._id}`}
            className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all ${t.isDeleted ? 'border-red-200 opacity-60' : 'border-gray-100'}`}>
            <div
              ...
              <div className='flex items-center gap-3'>
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${t.type === 'BILL' ? 'bg-indigo-100 text-indigo-600' : 'bg-green-100 text-green-600'}`}>
              ...
              <div className='text-right'>
                ...
                {t.type === 'BILL' && (t.paidAmount ?? 0) > 0 && (
                  <p className='text-[10px] text-green-500'>Paid: ₹{(t.paidAmount ?? 0).toLocaleString()}</p>
                )}
