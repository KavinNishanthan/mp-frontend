...
const UsersPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = { name: form.name, username: form.username, role: form.role, ...(form.password ? { password: form.password } : {}) };
      if (editingId) {
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-2 gap-4'>
        ...
        <div className='bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500'>
          ...
          <p className='text-2xl font-bold text-gray-800 mt-1'>{driversList.length}</p>
        </div>
      </div>
      ...
      <div className='bg-white rounded-lg shadow-lg overflow-hidden p-4'>
        <div className='overflow-x-auto'>
          <table className='min-w-full text-left text-sm'>
            ...
            <tbody>
              {users.map((user, i) => (
                <tr key={user._id} className={`${i % 2 ? 'bg-gray-100' : 'bg-white'}`}>
            ...
            <form onSubmit={handleSubmit} className='p-5 space-y-4'>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={editingId ? 'Leave blank to keep unchanged' : 'Set password'}
                />
              </div>
              ...
              <div>
                ...
                <select
                  value={form.role}
                  onChange={(e) => setForm({ ...form, role: e.target.value as 'admin' | 'driver' })}
                  className='w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500'>
