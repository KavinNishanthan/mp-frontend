...
const BillingPage = () => {
  ...
  const parseStockItems = (data: unknown): StockItem[] => {
    if (Array.isArray(data)) return data as StockItem[];
    if (data && typeof data === 'object' && 'items' in data) {
      const items = (data as { items: unknown }).items;
      return Array.isArray(items) ? (items as StockItem[]) : [];
    }
    return [];
  };
  ...
  const fetchVehicleStock = async (vId: string) => {
    ...
    try {
      const { data } = await stockApi.getVehicle(vId);
      setVehicleStock(parseStockItems(data));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [shopsRes, productsRes, vehiclesRes] = await Promise.all([
          shopsApi.getAll(),
          productsApi.getAll(),
          vehiclesApi.getAll(),
        ]);
        ...
        const myVehicle = vehicles.find(
          (v: { driverId?: { _id: string } | string; _id: string; registrationNumber?: string; name?: string }) =>
            (typeof v.driverId === 'object' && v.driverId !== null && v.driverId._id === user?._id) ||
            v.driverId === user?._id,
        );
        if (myVehicle) {
  ...
  const getProductName = (item: StockItem): string => {
    if (typeof item.productId === 'object' && item.productId !== null) return item.productId.name;
    const p = products.find((pr) => pr._id === item.productId);
    return p?.name || 'Unknown';
  };

  const getProductUnit = (item: StockItem): string => {
    if (typeof item.productId === 'object' && item.productId !== null) return item.productId.unit;
    const p = products.find((pr) => pr._id === item.productId);
    return p?.unit || '';
  };
  ...
  const addItem = () => {
    ...
    setBillItems([...billItems, { productId: product._id, productName: product.name, quantity: qty, price, total: price * qty }]);
  ...
  const handleSubmitBill = async () => {
    ...
    try {
      ...
      setPaidAmount(0);
      setSelectedShopId('');
      const { data: sData } = await shopsApi.getAll();
      ...
      const message =
        typeof error === 'object' && error !== null && 'response' in error
          ? (error as { response?: { data?: { message?: string } } }).response?.data?.message ?? 'Billing failed'
          : 'Billing failed';
      alert(message);
    } finally {
      setSubmitting(false);
            ...
            <div className='grid grid-cols-2 gap-2'>
              {vehicleStock
                ...
                .map((item, i) => (
                  <div
                    key={i}
                    className='bg-linear-to-br from-indigo-50 to-purple-50 rounded-lg px-3 py-2 border border-indigo-100/50'>
          ...
          <select
            ...
            {shops.map((s) => (
              <option key={s._id} value={s._id}>
                {s.name} (₹{Math.max(0, s.outstandingBalance)})
              </option>
            ))}
          </select>
          ...
          <div className='bg-white p-4 rounded-lg shadow-sm border border-gray-100 space-y-3'>
            ...
            <div className='space-y-2 mt-2'>
              {billItems.map((item, idx) => (
                <div
                  key={idx}
                  className='flex justify-between items-center bg-gray-50 p-3 rounded-lg text-sm border border-gray-100 group'>
                  <div>
        ...
        {billItems.length > 0 && (
          <div className='bg-white p-4 rounded-lg shadow-sm border border-gray-100 space-y-4 animate-slide-up'>
            ...
            <button
              ...
              className='btn-primary w-full bg-linear-to-r from-emerald-500 to-emerald-600 text-white font-semibold py-3 rounded-lg hover:from-emerald-600 hover:to-emerald-700 shadow-lg shadow-emerald-200 transition-all duration-200 active:scale-[0.98]'>
              Confirm Bill & Pay
            </button>
          </div>
        )}
      ...
      {showConfirm && (
        <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in'>
          <div className='bg-white rounded-lg shadow-2xl w-full max-w-sm overflow-hidden animate-scale-in'>
            <div className='bg-linear-to-r from-emerald-500 to-emerald-600 px-5 py-4 flex items-center justify-between'>
            ...
            <div className='px-5 pb-5 flex gap-3'>
              ...
              <button
                ...
                disabled={submitting}
                className='flex-1 py-2.5 bg-linear-to-r from-emerald-500 to-emerald-600 text-white rounded-lg text-sm font-bold hover:from-emerald-600 hover:to-emerald-700 transition-all disabled:opacity-50'>
