...
const VehiclesPage = () => {
  ...
  const fetchData = async () => {
    try {
      const [vehiclesRes, usersRes] = await Promise.all([vehiclesApi.getAll(), usersApi.getAll()]);
      setVehicles(vehiclesRes.data);
      setDrivers(usersRes.data.filter((u: { role: string }) => u.role === 'driver'));
    } catch (err) {
  ...
  const openEdit = (vehicle: Vehicle) => {
    setForm({ registrationNumber: vehicle.registrationNumber, name: vehicle.name ?? '' });
  ...
  const openAssign = (vehicle: Vehicle) => {
    ...
    setAssignDriverId(
      typeof vehicle.driverId === 'object' && vehicle.driverId ? vehicle.driverId._id : '',
    );
    setError('');
  ...
  const handleAssign = async () => {
    ...
    try {
      await vehiclesApi.assign(assignVehicleId, assignDriverId || null);
          ...
          <table className='min-w-full text-left text-sm'>
            ...
            <tbody>
              {vehicles.map((vehicle, i) => {
                ...
                return (
                  <tr key={vehicle._id} className={`${i % 2 ? 'bg-gray-100' : 'bg-white'}`}>
