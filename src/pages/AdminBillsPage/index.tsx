...
const AdminBillsPage = () => {
  ...
  const [auditLog, setAuditLog] = useState<
    { action: string; timestamp: string; adminId?: { name: string }; reason?: string; originalData?: { totalAmount: number; paidAmount: number }; modifiedData?: { totalAmount: number; paidAmount: number } }[] | null
  >(null);
  ...
  useEffect(() => {
    productsApi.getAll().then((r) => setProducts(r.data)).catch(console.error);
  }, []);
  ...
  const openEdit = (bill: Bill) => {
    ...
    setEditItems(
      bill.items.map((i) => ({
        productId: typeof i.productId === 'object' && i.productId !== null ? i.productId._id : (i.productId as string),
        productName: typeof i.productId === 'object' && i.productId !== null ? i.productId.name : '',
        quantity: i.quantity,
  ...
  const handleEditSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await billingApi.update(editingBill._id, {
  ...
  const handleDelete = async (billId: string) => {
    setSubmitting(true);
    try {
      await billingApi.remove(billId, deleteReason || 'Admin deletion');
  ...
  const viewAudit = async (billId: string) => {
    try {
      const { data } = await billingApi.getAuditLog(billId);
      setAuditLog(data);
    } catch (err) {
      console.error(err);
    }
  ...
  const filteredBills = bills.filter((b) => {
    if (searchShop) {
      const shopName = typeof b.shopId === 'object' && b.shopId !== null ? b.shopId.name : '';
      ...
      {loading ? (
        ...
        <div className='space-y-3'>
          ...
          {filteredBills.map((bill) => (
            <div
              key={bill._id}
              className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all ${bill.isDeleted ? 'border-red-200 opacity-60' : 'border-gray-100'}`}>
              <div
                  ...
                  <div className='min-w-0'>
                    <div className='flex items-center gap-2 flex-wrap'>
                      <span className='font-semibold text-sm text-gray-900 truncate'>
                        {typeof bill.shopId === 'object' && bill.shopId !== null ? bill.shopId.name : 'Shop'}
                      </span>
                      {bill.isDeleted && (
                    ...
                    <div className='text-xs text-gray-400 flex items-center gap-2 mt-0.5'>
                      ...
                      <span>• {typeof bill.driverId === 'object' && bill.driverId !== null ? bill.driverId.name : ''}</span>
                    </div>
                  </div>
                </div>
                <div className='text-right shrink-0 flex items-center gap-3'>
                  <div>
                    ...
                    {bill.paidAmount > 0 && (
                      <p className='text-[10px] text-green-500'>Paid: ₹{bill.paidAmount}</p>
                    )}
                  </div>
              ...
              {expandedBill === bill._id && (
                <div className='border-t border-gray-100 px-4 py-3 bg-gray-50/50 space-y-3'>
                  <div className='space-y-1'>
                    {bill.items.map((item, idx) => (
                      <div key={idx} className='flex justify-between text-xs text-gray-600'>
                        <span>
                          {typeof item.productId === 'object' && item.productId !== null ? item.productId.name : 'Product'} × {item.quantity} @ ₹{item.sellingPrice}
                        </span>
                  ...
                  <div className='text-[10px] text-gray-400'>
                    Vehicle: {typeof bill.vehicleId === 'object' && bill.vehicleId !== null ? bill.vehicleId.registrationNumber : ''} • Bill ID: {bill._id.slice(-6)}
                  </div>
                  {!bill.isDeleted && (
                    <div className='flex gap-2 pt-1'>
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setDeleteConfirm(bill._id); }}
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); viewAudit(bill._id); }}
      ...
      {editingBill && (
        <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto'>
            <div className='bg-linear-to-r from-blue-500 to-blue-600 px-5 py-4 flex items-center justify-between sticky top-0'>
              <h3 className='text-white font-bold text-lg flex items-center gap-2'>
                <Edit className='w-5 h-5' /> Edit Bill
              </h3>
              <button onClick={() => setEditingBill(null)} className='text-white/80 hover:text-white'>
                <X className='w-5 h-5' />
              </button>
            </div>
            <div className='p-5 space-y-4'>
              <div className='bg-gray-50 rounded-xl p-3'>
                <p className='text-xs text-gray-500'>
                  Shop: <span className='font-medium text-gray-800'>{typeof editingBill.shopId === 'object' && editingBill.shopId !== null ? editingBill.shopId.name : ''}</span>
                </p>
              </div>
              <div className='space-y-2'>
                ...
                {editItems.map((item, idx) => (
                  <div key={idx} className='flex gap-2 items-center bg-gray-50 p-2 rounded-lg'>
                    ...
                    <input
                      ...
                      min='1'
                      value={item.quantity}
                      className='w-14 text-xs border border-gray-200 rounded-lg p-1.5 text-center'
                    ...
                    <input
                      ...
                      className='w-16 text-xs border border-gray-200 rounded-lg p-1.5 text-center'
                ...
                <button
                  onClick={() => setEditItems([...editItems, { productId: products[0]?._id || '', quantity: 1, sellingPrice: products[0]?.defaultSellingPrice || 0, costPriceSnapshot: products[0]?.costPrice || 0, productName: products[0]?.name || '' }])}
            ...
            <div className='px-5 pb-5 flex gap-3'>
              <button onClick={() => setEditingBill(null)} className='flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50'>
                Cancel
              </button>
              <button
                onClick={handleEditSubmit}
                disabled={submitting}
                className='flex-1 py-2.5 bg-linear-to-r from-blue-500 to-blue-600 text-white rounded-xl text-sm font-bold hover:from-blue-600 hover:to-blue-700 disabled:opacity-50'>
      ...
      {deleteConfirm && (
        <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-sm'>
            <div className='bg-linear-to-r from-red-500 to-red-600 px-5 py-4 flex items-center justify-between'>
              <h3 className='text-white font-bold text-lg flex items-center gap-2'>
                <AlertTriangle className='w-5 h-5' /> Delete Bill
              </h3>
            ...
            <div className='p-5 space-y-4'>
              <p className='text-sm text-gray-600'>
                This bill will be soft-deleted. Outstanding balance will be adjusted automatically.
              </p>
              <div>
            ...
            <div className='px-5 pb-5 flex gap-3'>
              <button onClick={() => setDeleteConfirm(null)} className='flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600'>
                Cancel
              </button>
              <button
                onClick={() => handleDelete(deleteConfirm)}
                disabled={submitting}
                className='flex-1 py-2.5 bg-linear-to-r from-red-500 to-red-600 text-white rounded-xl text-sm font-bold disabled:opacity-50'>
      ...
      {auditLog && (
        <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
          <div className='bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto'>
            <div className='bg-linear-to-r from-gray-700 to-gray-800 px-5 py-4 flex items-center justify-between sticky top-0'>
              <h3 className='text-white font-bold text-lg flex items-center gap-2'>
                <Clock className='w-5 h-5' /> Audit Log
              </h3>
              <button onClick={() => setAuditLog(null)} className='text-white/80 hover:text-white'>
                <X className='w-5 h-5' />
              </button>
            </div>
            <div className='p-5 space-y-3'>
              {auditLog.length === 0 && (
                <p className='text-sm text-gray-400 text-center py-4'>No audit records</p>
              )}
              {auditLog.map((log, i) => (
                <div key={i} className='bg-gray-50 rounded-xl p-3 space-y-1.5'>
                  ...
                  {log.action === 'EDIT' && log.originalData && (
                    <div className='text-[10px] text-gray-400 space-y-0.5'>
                      ...
                      {log.modifiedData && (
                        <p>Modified: ₹{log.modifiedData.totalAmount} (Paid: ₹{log.modifiedData.paidAmount})</p>
                      )}
                    </div>
                  )}
                </div>
