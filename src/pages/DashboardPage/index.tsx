        ...
        {cards.map((card, i) => {
          ...
          return (
            <div
              ...
              <div
                className={`absolute -top-8 -right-8 w-24 h-24 rounded-full ${card.bgGlow} opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl`}
              />
...
const RecentBills = () => {
  const [bills, setBills] = useState<
    { _id: string; shopId?: { name: string }; date: string; totalAmount: number; paidAmount: number }[]
  >([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchBills = async () => {
      try {
        ...
        setBills(Array.isArray(data) ? data.slice(0, 5) : []);
      } catch {
        // Silently fail
      } finally {
        setLoading(false);
