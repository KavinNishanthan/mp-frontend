...
const DriverLayout = () => {
  ...
  return (
    <div className='min-h-screen bg-gray-50/50 flex flex-col'>
      <header className='bg-linear-to-r from-indigo-600 to-purple-600 text-white shadow-lg shadow-indigo-200/30 px-4 py-3 flex items-center justify-between sticky top-0 z-30'>
        <div className='flex items-center gap-3'>
          <div className='w-9 h-9 rounded-lg bg-white/20 backdrop-blur flex items-center justify-center'>
            <Milk className='w-5 h-5 text-white' />
          </div>
          <div>
            <h1 className='font-bold text-base leading-tight'>Kodai Diary Distributor</h1>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                {isActive && (
                  <div className='absolute -top-0.5 w-8 h-0.5 bg-linear-to-r from-indigo-500 to-purple-500 rounded-full' />
                )}
                <Icon className={clsx('w-5 h-5 mb-1', isActive && 'text-indigo-600')} />
                {item.name}
              </Link>
            );
          })}
